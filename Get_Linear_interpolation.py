...
def Get_Linear_interpolation_point(Pnt1=[1,1,30],Pnt2=[1,100,1],step=5):
    pass
...
def Get_Arc_interpolation_point(Pnt1=[],Pnt2=[],Pnt3=[],step=0.3):#pnt1为起点坐标，pnt2为终点坐标，pnt3为i,j,k
    try:
        ...
        if Pnt1[0] !=Pnt2[0]:#不是整圆
            ...
            if Pnt1[0] > Pnt2[0]:
                if Pnt1[1] > Pnt2[1]:
                    if i > 0:
                        Axis = gp_Dir(direct[0] * -1, direct[1] * -1, direct[2] * -1)  # G03
                    else:
                        Axis = gp_Dir(direct[0], direct[1], direct[2])  # G02
                else:
                    if i > 0:
                        Axis = gp_Dir(direct[0], direct[1], direct[2])  # G02
                    else:
